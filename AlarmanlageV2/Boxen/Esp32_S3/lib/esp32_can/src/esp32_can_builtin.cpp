...
//                                                                         tx,         rx,           mode
twai_general_config_t twai_general_cfg = TWAI_GENERAL_CONFIG_DEFAULT(GPIO_NUM_43, GPIO_NUM_44, TWAI_MODE_NORMAL);
...
void ESP32CAN::_init()
{
    ...
    bool debuggingMode = true; // Define the debuggingMode variable
    if (debuggingMode)
    ...
    if (!initializedResources)
    {
        ...
        if (debuggingMode)
            Serial.println("task rx created.");
        if (debuggingMode)
            Serial.println("task low level rx created.");
        xTaskCreatePinnedToCore(&CAN_WatchDog_Builtin, "CAN_WD_BI", 2048, this, 10, NULL, 1);
        if (debuggingMode)
            Serial.println("task watchdog created.");
...
uint32_t ESP32CAN::init(uint32_t ul_baudrate)
{
    ...
    bool debuggingMode = true; // Declare the debuggingMode variable
    if (debuggingMode)
    {
        // Reconfigure alerts to detect Error Passive and Bus-Off error states
        uint32_t alerts_to_enable = TWAI_ALERT_ERR_PASS | TWAI_ALERT_BUS_OFF | TWAI_ALERT_AND_LOG | TWAI_ALERT_ERR_ACTIVE | TWAI_ALERT_ARB_LOST | TWAI_ALERT_BUS_ERROR | TWAI_ALERT_TX_FAILED | TWAI_ALERT_RX_QUEUE_FULL;
...
bool ESP32CAN::processFrame(twai_message_t &frame)
{
    ...
    for (int i = 0; i < BI_NUM_FILTERS; i++)
    {
        ...
        if ((msg.id & filters[i].mask) == filters[i].id && (filters[i].extended == msg.extended))
        {
            ...
            bool debuggingMode = true; // Declare the variable 'debuggingMode' before using it
...
bool ESP32CAN::sendFrame(CAN_FRAME &txFrame)
{
    ...
    for (int i = 0; i < 8; i++)
        __TX_frame.data[i] = txFrame.data.byte[i];
